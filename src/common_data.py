...
def _collect_sections(root):
    ...
    for header in root.xpath('//h2[@id]'):
        ...
        if not content:
            raise ValueError('Section %s has no content in HTML dump.' % section_id)
...
def _extract_list_value(section_node, label):
    for item in section_node.xpath('.//li[strong]'):
        ...
        match = re.search(r'^%s\s*:\s*(.*)$' % re.escape(label), text, re.S)
        if match:
            value = unescape(_collapse_spaces(match.group(1)))
            if value:
                return value

    raise ValueError('Failed to extract %s information from HTML dump.' % label.lower())
...
def _extract_agenda(section_node):
    ...
    for row in rows:
        ...
        if time_text and desc_text:
            agenda.append('%s %s' % (time_text, desc_text))
...
def parse_common_data(event_url):
    ...
    def require(section_id):
        if section_id not in sections:
            raise ValueError('Failed to locate section %s in HTML dump.' % section_id)
...
def _main():
    parser = argparse.ArgumentParser(
        description='Dump the common_data section from a sciwork sprint HTML page.'
    )
    parser.add_argument(
        '--event-url',
        required=True,
